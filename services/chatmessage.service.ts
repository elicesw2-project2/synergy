...
class ChatMessageService {
  ...
  async createMessage(user_idx: number, ChatMessageInfo: ChatMessageInfo) {
    const { nickname, message, room_idx } = ChatMessageInfo;
    if (!nickname || !message || !room_idx) {
  ...
  async removeChat(user_idx: number, message_idx: number) {
    if (!message_idx) {
      throw new CustomError(400, 'message_idx 값이 없습니다');
    }
